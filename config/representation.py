...
class Representation():
    def __init__(self, screen, road, simulationManager):
        ...
        self.margins = (10, 10)
        self.cellSize = 15 #the height of road
        self.acc = 0
        self.theta = 0
...
class Representation():
    ...
    def __drawCell(self, x, y, speedLimit): #draws the road cells
     ...
     #   pygame.draw.rect(self.screen, (factor, factor, factor), (realPos[0], realPos[1], self.cellSize, self.cellSize), 0)
        pygame.draw.rect(self.screen, ( 180, 180, 180), (realPos[0], realPos[1], self.cellSize, self.cellSize), 2) 
...
#FOR HYDRODYNAMICS: Also talk about ants movements as AV model
        def __drawLane(self, lane, y, dt):
        x = 0
        for cell in lane: #cell stores the value (i.e. None if empty or car object if car object!) it is the element
            idx = x / self.cellSize, y / self.cellSize #each grid id in coordinates (x,y)
            speedLimit = self.road.getSpeedLimitAt(idx)
            self.__drawCell(x, y + mp.sin(5) + 100, speedLimit) 
            if cell != None:             
               # self.__drawCar(cell, x + 100*mp.cos(5*self.theta), y+ 100*mp.sin(5*self.theta)+100)
                self.theta += 5
            x += self.cellSize #the width of the road
'''
